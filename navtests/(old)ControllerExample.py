...
serial_port = '/dev/cu.usbmodem142101'  # -# Assign the location of the port that appears when the Arduino is plugged into the pi to a value
...
check_frequency = 3  # ~# Specify how many times a second to check for new values
...
while True:  # The program will check the button and joystick states until interrupted
    ...
    for current_axis in axes_to_check:  # Loop through the axes to check
        ...
        # if latest_value != 0.00 and abs(latest_value) > 0.1: print('Axis %i reads %.3f' % (current_axis, latest_value))
    ...
    if serial_is_on == True:
        print('line 61 ' + str(serial_output.encode()))
        ser.write(serial_output.encode())  # The text string is sent over serial, one character at a time
