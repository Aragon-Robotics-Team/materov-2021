        ...
        def loop(self):
            while True:
                ...
                if self.serialOn == True:
                    self.serialSendAndPrint(str(finallist[0]), str(finallist[1]), str(finallist[2]), str(finallist[3]))
                if j.get_button(8) == 1:
                    self.serialSendAndPrint(1500, 1500, 0, 0)
                    break
                pygame.event.clear()
                sleep(self.loopsleep)

        def serialSendAndPrint(self, w, x, y, z):
